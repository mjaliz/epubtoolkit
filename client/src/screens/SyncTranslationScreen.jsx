...
const SyncTranslationScreen = () => {
  ...
  const handleSubmit = async () => {
    if (bookFile !== null && translationFile !== null) {
      ...
      setLoading(true);
      try {
        const { data } = await http.post("/api/sync_translation", formData, {
  ...
  const handleDownload = async () => {
    const response = await http.get(
      `/api/download_synced_translation?book_path=${bookPath}`,
      {
        responseType: "blob",
      }
    );
  ...
  return (
    <div className="h-100 d-flex flex-column align-items-start justify-content-start py-5">
      ...
      <div className="input-group mb-3">
        <label className="input-group-text" htmlFor="translation">
          Translation
        </label>
        <input
          type="file"
